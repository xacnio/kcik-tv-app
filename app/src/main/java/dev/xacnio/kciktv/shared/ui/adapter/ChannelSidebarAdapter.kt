/**
 * File: ChannelSidebarAdapter.kt
 *
 * Description: RecyclerView Adapter for displaying Channel Sidebar lists.
 *
 * Author: Xacnio
 *
 */
...
class ChannelSidebarAdapter(
    ...
    override fun getItemCount(): Int = if (hasMore) channels.size + 1 else channels.size
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ChannelViewHolder) {
            ...
            if (!channel.isLive) {
                ...
                val localizedCat = dev.xacnio.kciktv.shared.util.CategoryUtils.getLocalizedCategoryName(holder.itemView.context, rawCat, channel.categorySlug)
            ...
            val defaultThumbnailBuilder = Glide.with(holder.itemView.context)
                .load(defaultThumb)
            ...
            val glideRequest = Glide.with(holder.itemView.context)
                .load(thumbUrl ?: defaultThumb)
                .placeholder(ShimmerDrawable(false))
    ...
    fun addChannels(newChannels: List<ChannelItem>, more: Boolean) {

        channels.addAll(newChannels)
        hasMore = more
        notifyDataSetChanged() // Easier to notify all when count logic changes
    }

    fun replaceChannels(newList: List<ChannelItem>, newIndex: Int, more: Boolean) {
        channels.clear()
