 ...
 *
 * Description: Implementation of Auth Repository functionality.
 *
 * Author: Xacnio
 *
 */
...
import dev.xacnio.kciktv.shared.data.api.LoginRequest
import dev.xacnio.kciktv.shared.data.api.MobileLoginRequest
...
class AuthRepository {
    ...
    suspend fun login(email: String, password: String, otp: String? = null): LoginResult = withContext(Dispatchers.IO) {
        try {
            ...
            val response = authService.mobileLogin(request = request)
            ...
            if (response.isSuccessful) {
                ...
                if (!token.isNullOrEmpty()) {
                    ...
                    if (userResponse?.profilePic.isNullOrEmpty()) {
                        try {
    ...
    /**
     * Fetches user info with a given token
     */
    suspend fun fetchUserInfo(token: String): UserResponse? = withContext(Dispatchers.IO) {
        try {
            val response = authService.getUser(token)
            if (response.isSuccessful) response.body() else null
        } catch (e: Exception) {
            Log.e("AuthRepository", "fetchUserInfo failed: ${e.message}")
            null
        }
    }
    /**
     * Fetches user kicks balance
     */
    suspend fun fetchKicksBalance(token: String): dev.xacnio.kciktv.shared.data.api.KicksBalanceData? = withContext(Dispatchers.IO) {
        try {
            val response = authService.getKicksBalance("Bearer $token")
            if (response.isSuccessful) response.body()?.data else null
        } catch (e: Exception) {
            Log.e(TAG, "fetchKicksBalance failed: ${e.message}")
            null
        }
    }
    /**
    ...
    suspend fun sendGift(
        ...
        giftId: String,
        message: String
    ): dev.xacnio.kciktv.shared.data.api.SendGiftResponse? = withContext(Dispatchers.IO) {
        try {
            val idempotencyKey = java.util.UUID.randomUUID().toString()
            val request = dev.xacnio.kciktv.shared.data.api.SendGiftRequest(giftId, message, idempotencyKey)
            val response = authService.sendGift("Bearer $token", channelId, request)
            ...
            if (response.isSuccessful) {
                ...
                if (!errorStr.isNullOrEmpty()) {
                    try {
                        ...
                        val details = dataObj?.optString("details")
                        
                        dev.xacnio.kciktv.shared.data.api.SendGiftResponse(
                            data = dev.xacnio.kciktv.shared.data.api.SendGiftData(null, null, details, null),
            ...
            Log.e(TAG, "sendGift failed: ${e.message}")
            null
        }
    }
}
